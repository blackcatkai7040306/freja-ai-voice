...
export const VoiceChat: React.FC = () => {
  ...
  const renderMessage = (message: VoiceMessage) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={clsx(
            "max-w-xs lg:max-w-md px-4 py-3 rounded-2xl shadow-lg backdrop-blur-md border-2",
            isUser
              ? "bg-gradient-to-br from-blue-600/95 to-purple-700/95 border-blue-300/40"
              : "bg-gray-900/95 border-gray-500/60"
          )}
          style={{
            color: "#393E46",
          }}
        >
          {/* Message content */}
          <div className="text-sm mb-2 font-semibold leading-relaxed">
            {message.content}
          </div>

          {/* Audio player for user messages */}
          {isUser && message.audioUrl && (
            <audio controls className="w-full mt-2 h-8 opacity-90">
          ...
          {!isUser && message.emotions && message.emotions.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {message.emotions.slice(0, 3).map((emotion) => (
                <span
                  key={emotion.name}
                  className="px-2 py-1 bg-gray-800/90 backdrop-blur-sm rounded-full text-xs text-gray-100 border border-gray-400/50 font-medium"
                  title={`${emotion.name}: ${(emotion.score * 100).toFixed(
                    0
                  )}%`}
                >
          ...
          {/* Timestamp */}
          <div className="text-xs opacity-80 mt-1 font-mono text-gray-200">
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white relative">
      {/* Background Image optimized for face visibility */}
      <div
        ...
        style={{
          ...
          filter: "blur(1.5px)", // Reduced blur for better facial detail
          transform: "scale(1.02)", // Minimal scale to prevent edge gaps
        }}
      />

      {/* Gradient overlay for better text readability and face emphasis */}
            ...
            <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-gray-100 max-w-md leading-relaxed backdrop-blur-sm bg-black/40 rounded-lg p-4 font-medium border border-gray-600/30">
                    {isConnected
        ...
        <div className="p-6 bg-gray-900/90 backdrop-blur-sm border-t border-gray-800/50">
          ...
          <div className="flex flex-col items-center mt-4 space-y-2">
            {conversationState.isPlaying && (
              <div className="flex items-center justify-center text-green-300 text-sm bg-black/50 backdrop-blur-sm rounded-lg px-3 py-2 border border-green-400/30 font-medium">
            ...
            {!isConnected && (
              <div className="flex items-center justify-center text-red-300 text-sm bg-black/50 backdrop-blur-sm rounded-lg px-3 py-2 border border-red-400/30 font-medium">
