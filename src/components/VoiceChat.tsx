      ...
      >
        ...
        {!isUser && (
          <div className="flex flex-col items-center space-y-1">
            ...
            <span className="text-xs text-gray-300 font-semibold hidden sm:block">{assistantName}</span>
          </div>
        )}
        ...
        <div
          className={clsx(
            ...
            isUser
              ? 'bg-gradient-to-br from-blue-600/95 to-purple-700/95 text-white border border-blue-400/50'
              : 'bg-gray-900/95 text-gray-100 border border-gray-500/60'
          )}
        >
          {/* Message content */}
          <div className={clsx(
            'text-sm md:text-base mb-1 md:mb-2 font-semibold leading-relaxed',
          ...
          {!isUser && message.emotions && message.emotions.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {message.emotions.slice(0, 3).map((emotion) => (
                <span
                  key={emotion.name}
                  className="px-2 py-1 bg-gray-800/80 backdrop-blur-sm rounded-full text-xs text-gray-50 border border-gray-500/60 font-medium"
          ...
          <div className={clsx(
            ...
            isUser ? 'text-blue-100/80' : 'text-gray-300/90'
          )}>
        ...
        {isUser && (
          <div className="flex flex-col items-center space-y-1">
            ...
            <span className="text-xs text-gray-300 font-semibold hidden sm:block">{userName}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white relative">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {/* Settings panel */}
        {showSettings && (
          <div className="bg-gray-900/95 backdrop-blur-sm border-b border-gray-700/60 p-3 md:p-4 space-y-3 md:space-y-4">
            <h3 className="text-sm font-bold text-gray-100">Voice Settings</h3>
            
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              {/* Microphone toggle */}
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-100 font-medium">Microphone</span>
                <button
              ...
              {/* Speaker toggle */}
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-100 font-medium">Speaker</span>
                <button
                  onClick={() => updateVoiceSettings({ 
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-100 font-medium">Volume</span>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-100 font-medium">Speech Speed</span>
                <span className="text-xs text-gray-200 font-semibold">
            ...
            {/* Auto-play toggle */}
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-100 font-medium">Auto-play responses</span>
          ...
          {conversationState.messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-4 md:space-y-6 px-4">
              <div className="space-y-3 md:space-y-4">
                ...
                <div className="space-y-2">
                  <h2 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-blue-300 to-purple-300 bg-clip-text text-transparent drop-shadow-sm">
                    Welcome, Robert!
                  </h2>
                  <p className="text-gray-100 max-w-sm md:max-w-md leading-relaxed backdrop-blur-sm bg-black/40 rounded-lg p-3 md:p-4 text-sm md:text-base font-medium border border-gray-600/30">
                    {isConnected 
        ...
        <div className="p-4 md:p-6 bg-gray-900/90 backdrop-blur-sm border-t border-gray-800/50">
          ...
          <div className="flex flex-col items-center mt-3 md:mt-4 space-y-2">
            {conversationState.isPlaying && (
              <div className="flex items-center justify-center text-green-300 text-sm bg-black/50 backdrop-blur-sm rounded-lg px-3 py-2 border border-green-500/30">
                <Volume2 className="w-4 h-4 mr-2" />
                <span className="hidden sm:inline font-medium">Playing response...</span>
                <span className="sm:hidden font-medium">Playing...</span>
              </div>
            )}
            
            {!isConnected && (
              <div className="flex items-center justify-center text-red-300 text-sm bg-black/50 backdrop-blur-sm rounded-lg px-3 py-2 border border-red-500/30">
                <WifiOff className="w-4 h-4 mr-2" />
                <span className="hidden sm:inline font-medium">Not connected to Hume AI</span>
                <span className="sm:hidden font-medium">Not connected</span>
              </div>
            )}
          </div>
        </div>
